...
class SWSH_Egg_1BOX(ImageProcPythonCommandTrim):
    ...
    def do(self):
        ...
        self.hatched_egg_total = int(ret[7])
        self.shiny_total = int(ret[8])
        ...
        before_time = start_time.time()
        now_time = start_time.time()
        print("スタート！")

        try:
            while self.shiny_total < max_count:
                ...
                while count < 6:
                    ...
                    self.hatching()
                    self.putHatchedEggs(count+1) # 色違いがいた場合は自動的にリセットされる
                    if self.hasShiny: # 色違いがいた場合はリセットしているため、色卵を退避させる
                    ...
                    self.report()
                    count += 1
                self.releasePokemon1Box() # 1ボックス分のタマゴを孵化させた後、ポケモンを逃がす
                self.openBox()
                self.pickUpPokemons()
                self.closeMenu()
                self.report()
                cycle_end_time = datetime.datetime.now()
                self.jprint_append('育て屋に戻って孵化厳選を再開します。')
                self.jprint_append(f'【孵化総数】{self.shiny_total}/{self.hatched_egg_total}')
                self.jprint_append(f'【退避数(色違い・理想個体)】{self.moveNum}')
                self.jprint_append(f'【サイクル実行時間】{cycle_end_time - cycle_start_time}')
    ...
    def checkBest(self,y,h,x,w):
        if self.isContainTemplate(f'SWSH/EGG_Util/{self.lang}/statusBest.png', 0.75, use_gray=True, show_value=self.show_value, area=[y,y+h,x,x+w]):
            return True
        else:
            return False
    ...
    def checkNoGood(self,y,h,x,w):
        if self.isContainTemplate(f'SWSH/EGG_Util/{self.lang}/statusNoGood.png', 0.75, use_gray=True, show_value=self.show_value, area=[y,y+h,x,x+w]):
            return True
        else:
            return False
    ...
    # 色違い判定
    def isShiny(self):
        if self.isContainTemplate(f'SWSH/EGG_Util/shiny.png', 0.75, use_gray=True, show_value=self.show_value, area=[93,145,1224,1280]):
    ...
    # 色卵や理想個体のポケモンを空いている位置に移動する（ボックスでポケモンにカーソルを合わせた状態）
    def movePokemonToEmpty(self,row,col):
        self.press(Button.A, 0.1, 1)
        self.press(Button.A, 0.1, 1)
        self.moveCellFocus(row, col, 1, 1)
    ...
    def set_param(self):
        ...
        while True:
            dialogue_list = [
                ...
                ['Entry','途中から：累計孵化数',egg_total], #7
                ['Entry','途中から：累計色違い数',shiny_total] #8
            ]
            ret = self.dialogue6widget("BDSP自動孵化設定", dialogue_list)

            # ret[7]とret[8]が0以上の整数に変換できるか確認
            try:
                if int(ret[7]) < 0 or int(ret[8]) < 0:
                    self.jprint("累計数には0以上の整数を入力してください。")
                    continue
            except ValueError:
                self.jprint("累計数には0以上の整数を入力してください。")
                continue
